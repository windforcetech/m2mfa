...
public interface MesMoScheduleRepository extends BaseRepository<MesMoSchedule,String> {
    ...
    @Query(value = "select MAX(sequence)  from mes_mo_schedule where  machine_id=?1 and flag !=MoScheduleStatus.CLOSE and flag !=MoScheduleStatus.FORCECLOSE",nativeQuery = true)
