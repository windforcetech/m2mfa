...
public class BaseStaffshiftServiceImpl implements BaseStaffshiftService {
    ...
    public PageUtil<Map<String, Object>> list(BaseStaffshiftQuery query) {
        ...
        String sql = " SELECT zh.* , base_staff.staff_name , base_staff.code   from     (SELECT staff_id, CAST( GROUP_CONCAT( shift_date, ';', id, ';',shift_name ORDER BY shift_date ASC SEPARATOR '|' ) AS CHAR (10000) CHARACTER SET utf8 ) as keyvalue FROM  (SELECT base_staffshift.id,staff_id, shift_date,base_shift.shift_id, base_shift.`name` as shift_name  from base_staffshift,base_shift where staff_id in (SELECT staff_id FROM `base_staff` where 1=1 ";
