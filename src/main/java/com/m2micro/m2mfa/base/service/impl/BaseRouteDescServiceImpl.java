...
public class BaseRouteDescServiceImpl implements BaseRouteDescService {
    ...
    public PageUtil<BaseRouteDesc> list(BaseRouteQuery query) {
        ...
        if (StringUtils.isNotEmpty(query.getInputProcess())) {
            QBaseProcess qBaseProcess = QBaseProcess.baseProcess;
            BooleanBuilder condition1 = new BooleanBuilder();
            condition1.and(qBaseProcess.processName.like("%" + query.getInputProcess() + "%"));

            List<String> collect = Lists.newArrayList(baseProcessService.findAll(condition1))
                    .stream().map(BaseProcess::getProcessId).collect(Collectors.toList());
            condition.and(qBaseRouteDesc.inputProcess.in(collect));
        }
        if (StringUtils.isNotEmpty(query.getOutputProcess())) {
            QBaseProcess qBaseProcess = QBaseProcess.baseProcess;
            BooleanBuilder condition1 = new BooleanBuilder();
            condition1.and(qBaseProcess.processName.like("%" + query.getOutputProcess() + "%"));

            List<String> collect = Lists.newArrayList(baseProcessService.findAll(condition1))
                    .stream().map(BaseProcess::getProcessId).collect(Collectors.toList());
            condition.and(qBaseRouteDesc.outputProcess.in(collect));
        }
